...
const browser = await puppeteer.launch({ executablePath: CHROME, headless: false, args: ['--no-sandbox', '--disable-dev-shm-usage', '--start-maximized'] });
const page = await browser.newPage();
await page.setViewport({ width: 1440, height: 900 });
const t0 = Date.now();
await page.goto(GAS, { waitUntil: 'load', timeout: 180000 }).catch(() => {});
...
while (!app && Date.now() - t0 < 150000) { await sleep(2000); app = await pickAppFrame(); }
if (!app) { const urls = page.frames().map(f=>f.url()); console.log('NO FRAME. frames: ' + JSON.stringify(urls)); await browser.close(); process.exit(0); }
