  ...
  for (let i = 0; i < 30; i++) {
    ...
    const b = await app.evaluate(() => {
      ...
      return {
        ...
        acDisplay: ac ? getComputedStyle(ac).display : 'none-el',
        loginDisplay: lp ? getComputedStyle(lp).display : 'none-el',
        activePage: active ? active.id : 'NONE',
        curUser: typeof window.currentUser === 'undefined' ? 'undef' : (window.currentUser ? (window.currentUser.name || window.currentUser.email || 'set') : 'null')
      };
    }).catch(() => null);
    if (b) { console.log('boot ' + JSON.stringify(b)); bootSamples.push(b); }
  }
})();
await sleep(3000);
