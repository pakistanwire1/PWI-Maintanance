...
const browser = await puppeteer.launch({ executablePath: CHROME, headless: 'new', userDataDir: 'C:/Users/afsar/AppData/Local/Temp/opencode/badge6_chrome', args: ['--no-sandbox', '--disable-dev-shm-usage', '--window-size=1440,1000', '--incognito'] });
...
const res = await page.evaluate(() => {
  const out = {};
  out.badgeRefreshSrc = Badge.refresh ? Badge.refresh.toString().slice(0, 700) : 'MISSING';
  try { out.user = Session.getUser(); } catch (e) { out.userErr = String(e); }
  try { out.userEmailGetter = Session.getUserEmail ? Session.getUserEmail() : 'n/a'; } catch (e) { out.getErr = String(e); }
  return out;
});
