...
const pairs = {};
for (const r of rows) {
  const d = r.Department || r.department || '(none)';
  const m = r.MachineNumber || r.machineNumber || '';
  if (!m) continue;
  const k = d + ' + ' + m;
  pairs[k] = (pairs[k] || 0) + 1;
}
const sorted = Object.entries(pairs).sort((a,b) => b[1]-a[1]);
console.log('dept+machine combos:');
for (const [k,v] of sorted.slice(0, 15)) console.log('  ' + k + ' x' + v);
