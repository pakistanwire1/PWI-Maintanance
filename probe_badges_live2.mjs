...
const browser = await puppeteer.launch({ executablePath: CHROME, headless: 'new', userDataDir: 'C:/Users/afsar/AppData/Local/Temp/opencode/badge2_chrome', args: ['--no-sandbox', '--disable-dev-shm-usage', '--window-size=1440,1000', '--incognito'] });
const page = await browser.newPage();
await page.setViewport({ width: 1440, height: 900 });
page.on('console', m => { const t = m.type(); if (t === 'error') console.log('[console:error]', m.text().slice(0, 300)); });
...
const snap = async (tag) => {
  const r = await page.evaluate(() => {
    const ids = ['emailBadge', 'waBadge', 'notificationBadge', 'badge-openjobcard', 'badge-startjobcard', 'badge-closejobcard', 'badge-pendingjobcard', 'badge-approvejobcard'];
    const o = {};
...
await snap('t8s');
await page.evaluate(() => { if (window.Badge && Badge.refresh) Badge.refresh(); });
await sleep(6000);
await snap('after-manual-refresh');
await page.screenshot({ path: 'C:/Users/afsar/AppData/Local/Temp/opencode/badge_live2_topbar.png' });
await browser.close();
